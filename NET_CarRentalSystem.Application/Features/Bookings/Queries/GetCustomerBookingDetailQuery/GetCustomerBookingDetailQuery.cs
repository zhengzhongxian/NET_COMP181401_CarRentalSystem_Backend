...
namespace NET_CarRentalSystem.Application.Features.Bookings.Queries.GetCustomerBookingDetailQuery;

public class GetCustomerBookingDetailQuery : IQuery<GetBookingDetailDto?>
{
    ...
    ICryptographyService cryptographyService) : IRequestHandler<GetCustomerBookingDetailQuery, GetBookingDetailDto?>
{
    public async Task<GetBookingDetailDto?> Handle(GetCustomerBookingDetailQuery request, CancellationToken cancellationToken)
    {
        ...
        const string sql = """
            ...
            WHERE booking_id_src = @BookingId AND customer_id = @CustomerId AND is_deleted = 0
        """;

        var bookingFlat = await dapperRepository.QueryFirstOrDefaultAsync<GetBookingReadFlatDto>(
            sql, 
            new { request.BookingId, customer.CustomerId }, 
