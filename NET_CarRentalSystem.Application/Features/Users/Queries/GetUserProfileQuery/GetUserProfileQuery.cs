    ...
    ILogger<GetUserProfileQueryHandler> logger) : IRequestHandler<GetUserProfileQuery, GetUserDto>
{
    public async Task<GetUserDto> Handle(GetUserProfileQuery request, CancellationToken cancellationToken)
    {
        ...
        var userDto = new GetUserDto
        {
            ...
            PhoneNumber = cryptographyService.SafeDecryptAes(customer?.PhoneNumber),
            Address = customer?.Address,
            Dob = customer?.Dob,
            CccdNumber = cryptographyService.SafeDecryptAes(customer?.CccdNumber),
            ...
            DriverLicenseId = cryptographyService.SafeDecryptAes(customer?.DriverLicenseId),
